...
(function () {
  ...
  if (backAvailable()) {
    ...
    // Гипотеза на баг «кнопка "назад" самого Telegram перестаёт работать,
    // помогает только перезагрузка» (жалоба пользователя 31.07.2026): при
    // навигации браузера НАЗАД страница нередко восстанавливается из
    // bfcache — JS-контекст замораживается/размораживается как есть, без
    // повторного исполнения скриптов. Похоже, нативная сторона Telegram
    // WebView при этом теряет синхронизацию с уже устаревшим (по её мнению)
    // состоянием кнопки — pageshow с persisted=true ловит именно такое
    // восстановление, и на нём пересчитываем видимость кнопки заново.
    // Не подтверждено на устройстве — требует проверки в реальном Telegram.
    window.addEventListener("pageshow", function (e) {
      if (e.persisted) updateBackButton();
    });
  }
