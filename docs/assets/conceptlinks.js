...
(function () {
  "use strict";
  var container = document.querySelector(".note-body, .fulltext-body");
  ...
  Object.keys(keywords).forEach(function (phrase) {
    ...
    // Уже есть ссылка на эту же карту на странице (например, в «Дальше по
    // мысли» или в самом «Источнике») — не дублируем ещё одной в прозе.
    if (container.querySelector('a[href="' + url + '"]') || document.querySelector('.note-source a[href="' + url + '"]')) return;
